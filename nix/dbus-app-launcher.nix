{
  "0.1.0.0" = {
    sha256 = "bb09fc801920df9d217a0993064c7848a9811d8b24bd26119aa963f327305490";
    revisions = {
      r0 = {
        nix = import ../hackage/dbus-app-launcher-0.1.0.0-r0-c5c1748f1a49b83b5c5b20ed0ebd11eb5b5ccadc24eb32aa3b9b40afb1cd09ed.nix;
        revNum = 0;
        revTimestamp = "2024-03-24T13:12:36Z";
        sha256 = "c5c1748f1a49b83b5c5b20ed0ebd11eb5b5ccadc24eb32aa3b9b40afb1cd09ed";
        };
      default = "r0";
      };
    };
  "0.1.1.0" = {
    sha256 = "1c7b0bd23fe44b58de0de95e802f72d491f37bd34ea45e1a37e14a60828bea43";
    revisions = {
      r0 = {
        nix = import ../hackage/dbus-app-launcher-0.1.1.0-r0-34df40e526d5b970c52e1368867ac141259f60718e47790c0c3692a140d18999.nix;
        revNum = 0;
        revTimestamp = "2024-12-14T17:48:31Z";
        sha256 = "34df40e526d5b970c52e1368867ac141259f60718e47790c0c3692a140d18999";
        };
      default = "r0";
      };
    };
  }