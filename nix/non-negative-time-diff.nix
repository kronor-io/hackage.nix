{
  "0.0.1" = {
    sha256 = "355282cb16db376223d771941d085004460d04e0097c9b84ad313bf524527cea";
    revisions = {
      r0 = {
        nix = import ../hackage/non-negative-time-diff-0.0.1-r0-a6f85e4aa05f637a7b49c1ac90821f240aa14301c40793cd70f5d9bac474691e.nix;
        revNum = 0;
        revTimestamp = "2026-03-09T13:54:35Z";
        sha256 = "a6f85e4aa05f637a7b49c1ac90821f240aa14301c40793cd70f5d9bac474691e";
      };
      default = "r0";
    };
  };
  "0.0.2" = {
    sha256 = "f7dc76d17ae79349e614e36f332a55a55b8ae3a47fc1f68cba05257617d181ca";
    revisions = {
      r0 = {
        nix = import ../hackage/non-negative-time-diff-0.0.2-r0-4ccd583734bccb6d972c24f7f9ac5063423190620e18c6b19f5c9194d3b28e01.nix;
        revNum = 0;
        revTimestamp = "2026-03-14T12:57:42Z";
        sha256 = "4ccd583734bccb6d972c24f7f9ac5063423190620e18c6b19f5c9194d3b28e01";
      };
      default = "r0";
    };
  };
}