{
  "1.0.1" = {
    sha256 = "c696f2c60e3ad66b7867439b2314a288c17499601ec3276122a8e9cafa56c6f0";
    revisions = {
      r0 = {
        nix = import ../hackage/gi-dazzle-1.0.1-r0-e78300f071bbcafa9bfddd48f0ea89edd5756f7030db39e73627c89647548830.nix;
        revNum = 0;
        revTimestamp = "2024-06-09T12:39:19Z";
        sha256 = "e78300f071bbcafa9bfddd48f0ea89edd5756f7030db39e73627c89647548830";
        };
      default = "r0";
      };
    };
  "1.0.2" = {
    sha256 = "e6a9d8bf8cad75a48186575833a2264b13623a14168ab1c6c035a04697304060";
    revisions = {
      r0 = {
        nix = import ../hackage/gi-dazzle-1.0.2-r0-e0a2da33e71f05cf93280db04119614d7a4fab0b42f2c3100c600797e8a409ce.nix;
        revNum = 0;
        revTimestamp = "2024-06-16T21:12:45Z";
        sha256 = "e0a2da33e71f05cf93280db04119614d7a4fab0b42f2c3100c600797e8a409ce";
        };
      default = "r0";
      };
    };
  "1.0.3" = {
    sha256 = "6c25fa55fc23d96dc385bc38ef07de0a646f7a7034570fce1d99cd2291f8e772";
    revisions = {
      r0 = {
        nix = import ../hackage/gi-dazzle-1.0.3-r0-ca99fc6d2c84641e195cf982df087662f6351da314f4c271ee7e932375ae529b.nix;
        revNum = 0;
        revTimestamp = "2025-01-01T12:36:10Z";
        sha256 = "ca99fc6d2c84641e195cf982df087662f6351da314f4c271ee7e932375ae529b";
        };
      default = "r0";
      };
    };
  }