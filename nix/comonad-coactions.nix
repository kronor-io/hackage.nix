{
  "0.1.0.0" = {
    sha256 = "38bf1dc7c5839f6f5cd1111bf9801973881e018c5198bc990ff5bea789dffaf8";
    revisions = {
      r0 = {
        nix = import ../hackage/comonad-coactions-0.1.0.0-r0-a68eea45f7919351f52d8a870ca7847161e8f6d9753c81ad49ba4e77d287bbd5.nix;
        revNum = 0;
        revTimestamp = "2026-02-09T04:30:21Z";
        sha256 = "a68eea45f7919351f52d8a870ca7847161e8f6d9753c81ad49ba4e77d287bbd5";
      };
      default = "r0";
    };
  };
  "0.1.0.1" = {
    sha256 = "b6b838f8c99760b32955c9c25cfdb1830e144b68b1a2186fc284a58c13c1e275";
    revisions = {
      r0 = {
        nix = import ../hackage/comonad-coactions-0.1.0.1-r0-321e02e8d6eaa17196d4e02f8ca032428a45ddf8216c7d47d39d138f7521d8df.nix;
        revNum = 0;
        revTimestamp = "2026-04-19T01:00:39Z";
        sha256 = "321e02e8d6eaa17196d4e02f8ca032428a45ddf8216c7d47d39d138f7521d8df";
      };
      default = "r0";
    };
  };
}