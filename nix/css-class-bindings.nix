{
  "0.0.2" = {
    sha256 = "ae4343abee1a6873b085a039424e1391798582e66ecef61343c2010f6be4e2df";
    revisions = {
      r0 = {
        nix = import ../hackage/css-class-bindings-0.0.2-r0-db6ce7a4cc6352f96f387d34e12e13ddf8555f400389a6586a8d62a3f75d042f.nix;
        revNum = 0;
        revTimestamp = "2026-03-28T19:24:23Z";
        sha256 = "db6ce7a4cc6352f96f387d34e12e13ddf8555f400389a6586a8d62a3f75d042f";
      };
      default = "r0";
    };
  };
  "0.0.3" = {
    sha256 = "3f9eeaedae8efd233deaae6ee845b4c1afad64ee5871aa88e5a1787fab8a23e4";
    revisions = {
      r0 = {
        nix = import ../hackage/css-class-bindings-0.0.3-r0-3d3470554c9e93f954d501b87d6715526a60325234ae969e08be5018fd2c8ea8.nix;
        revNum = 0;
        revTimestamp = "2026-03-30T08:08:00Z";
        sha256 = "3d3470554c9e93f954d501b87d6715526a60325234ae969e08be5018fd2c8ea8";
      };
      default = "r0";
    };
  };
  "0.0.4" = {
    sha256 = "c3cc20236ce0640505a70f24485af6c4cc4937593f5ec35520bd09bdcbbdf301";
    revisions = {
      r0 = {
        nix = import ../hackage/css-class-bindings-0.0.4-r0-0882e6b537e281a5866ed02c43e26b4bd479385e33abf0cf14fb37661eb7b864.nix;
        revNum = 0;
        revTimestamp = "2026-04-07T14:56:32Z";
        sha256 = "0882e6b537e281a5866ed02c43e26b4bd479385e33abf0cf14fb37661eb7b864";
      };
      default = "r0";
    };
  };
}