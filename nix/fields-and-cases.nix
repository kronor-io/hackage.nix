{
  "0.1.0.0" = {
    sha256 = "ae766c46777abb276a217a6207002b0033ee328a1378f8748003698dd3eb83b4";
    revisions = {
      r0 = {
        nix = import ../hackage/fields-and-cases-0.1.0.0-r0-aa8347d1e6a1df8fd2cb28f5bef03240a68f90ad5977f1100de73e367be66ee7.nix;
        revNum = 0;
        revTimestamp = "2024-07-12T19:30:55Z";
        sha256 = "aa8347d1e6a1df8fd2cb28f5bef03240a68f90ad5977f1100de73e367be66ee7";
        };
      default = "r0";
      };
    };
  "0.2.0.0" = {
    sha256 = "bc89c7e8f2f970a489373bda1ba26fa9dfa6656a258f187888fb0fa364eb832f";
    revisions = {
      r0 = {
        nix = import ../hackage/fields-and-cases-0.2.0.0-r0-4540009e1b29d658a4b43b4cc7191ffcec8929c2384936b447b02abb30714b53.nix;
        revNum = 0;
        revTimestamp = "2024-08-16T19:31:09Z";
        sha256 = "4540009e1b29d658a4b43b4cc7191ffcec8929c2384936b447b02abb30714b53";
        };
      default = "r0";
      };
    };
  }