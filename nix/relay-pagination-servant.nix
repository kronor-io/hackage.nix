{
  "0.1.0.0" = {
    sha256 = "afd0bdc08d85f40a9e2339d9d25799f78b44e987ee99eb02d9f4939256189843";
    revisions = {
      r0 = {
        nix = import ../hackage/relay-pagination-servant-0.1.0.0-r0-35c4b01ddf34aa1a0987608906426cdcd78db786abf8632011ec55a4be9c58c2.nix;
        revNum = 0;
        revTimestamp = "2026-07-16T23:06:18Z";
        sha256 = "35c4b01ddf34aa1a0987608906426cdcd78db786abf8632011ec55a4be9c58c2";
      };
      default = "r0";
    };
  };
  "0.1.1.0" = {
    sha256 = "ad29a356533ecaaefbf680b91c2894fbbd5c396806bbece1fc0e428fe557cf5d";
    revisions = {
      r0 = {
        nix = import ../hackage/relay-pagination-servant-0.1.1.0-r0-699866f29f09cacfcca2ce9644ffa63d0618fd80e92f1a9ddcd977187134fae3.nix;
        revNum = 0;
        revTimestamp = "2026-07-24T21:47:19Z";
        sha256 = "699866f29f09cacfcca2ce9644ffa63d0618fd80e92f1a9ddcd977187134fae3";
      };
      default = "r0";
    };
  };
}