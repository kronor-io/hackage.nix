{
  "0.1.0.0" = {
    sha256 = "ce9046971dc391b63eb67a7202357c056cd25787646524de0d461b1ffa4ff4d6";
    revisions = {
      r0 = {
        nix = import ../hackage/moonlight-linalg-0.1.0.0-r0-4a1b3735bfe80acfe20ee18294a8f901b23dd9d9a1cfe4b6aae0e08cb5181905.nix;
        revNum = 0;
        revTimestamp = "2026-08-21T23:47:19Z";
        sha256 = "4a1b3735bfe80acfe20ee18294a8f901b23dd9d9a1cfe4b6aae0e08cb5181905";
      };
      default = "r0";
    };
  };
  "0.1.0.1" = {
    sha256 = "f9442b42dbd25f4163440c547345fe46e5ea806d7d8a24f3fa31b7614ecd8167";
    revisions = {
      r0 = {
        nix = import ../hackage/moonlight-linalg-0.1.0.1-r0-560d2f34468c68f6022f2d6533cb7c42c8e808f40b89bffbd67457a7bfcc3ec0.nix;
        revNum = 0;
        revTimestamp = "2026-08-22T01:26:02Z";
        sha256 = "560d2f34468c68f6022f2d6533cb7c42c8e808f40b89bffbd67457a7bfcc3ec0";
      };
      default = "r0";
    };
  };
}