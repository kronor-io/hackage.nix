{
  "0.1.0.0" = {
    sha256 = "daa67c3ec614fa87c9b4d7361e3bd3c68c7059ee477a0918175212838c62ee05";
    revisions = {
      r0 = {
        nix = import ../hackage/fuyu-gpio-direct-0.1.0.0-r0-f84bd2112d230c936b3e2f05d9ab5fe2912644f739439c47f2eea4765eb95d43.nix;
        revNum = 0;
        revTimestamp = "2026-08-05T00:35:10Z";
        sha256 = "f84bd2112d230c936b3e2f05d9ab5fe2912644f739439c47f2eea4765eb95d43";
      };
      default = "r0";
    };
  };
  "0.2.0.0" = {
    sha256 = "cf085ca323fbc090d1446e4c0fa96ec9c8d112167edaf15ab243afacd53b0a6b";
    revisions = {
      r0 = {
        nix = import ../hackage/fuyu-gpio-direct-0.2.0.0-r0-2e8e981beb495929e5638ae2ad3afaecc30cff2bf7c9a8d291756b5f9fe058c8.nix;
        revNum = 0;
        revTimestamp = "2026-09-06T06:01:30Z";
        sha256 = "2e8e981beb495929e5638ae2ad3afaecc30cff2bf7c9a8d291756b5f9fe058c8";
      };
      default = "r0";
    };
  };
}