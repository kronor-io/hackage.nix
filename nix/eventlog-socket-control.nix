{
  "0.1.0.0" = {
    sha256 = "b936ec96248e7f5470d72ae1975db03b8dba502998817222902f4f0fdb49687e";
    revisions = {
      r0 = {
        nix = import ../hackage/eventlog-socket-control-0.1.0.0-r0-94a94f0f12bb8b40b65f8278ffe2fcb6b52bc0dfb531466e12083a82aaa52ce1.nix;
        revNum = 0;
        revTimestamp = "2026-03-04T15:02:14Z";
        sha256 = "94a94f0f12bb8b40b65f8278ffe2fcb6b52bc0dfb531466e12083a82aaa52ce1";
      };
      default = "r0";
    };
  };
  "0.1.1.0" = {
    sha256 = "6fb480099d06e1ec0f45451279f392664aafe6a327b354342e394560fdfea929";
    revisions = {
      r0 = {
        nix = import ../hackage/eventlog-socket-control-0.1.1.0-r0-24349a56d3261c844ab5d81729b29350074d02eaa92ec7f167cdfedefbdccf62.nix;
        revNum = 0;
        revTimestamp = "2026-04-10T13:24:13Z";
        sha256 = "24349a56d3261c844ab5d81729b29350074d02eaa92ec7f167cdfedefbdccf62";
      };
      default = "r0";
    };
  };
}