{
  "0.1.0.0" = {
    sha256 = "ba83accdc042f8a5b5518e55a7b9c9e792e83b9a229c47bf68c0fbde8552bf83";
    revisions = {
      r0 = {
        nix = import ../hackage/circuit-notation-0.1.0.0-r0-bd0c416b5e0766006bbd7a07b1ce3f6a7f35b84216a4d77172362d3de8b960c9.nix;
        revNum = 0;
        revTimestamp = "2024-07-30T22:04:18Z";
        sha256 = "bd0c416b5e0766006bbd7a07b1ce3f6a7f35b84216a4d77172362d3de8b960c9";
      };
      default = "r0";
    };
  };
  "0.2.0.0" = {
    sha256 = "e3eeb0cba9056af15c9a1a2d62f0bb9e2be888bfed8d6601019a8feb63bf9717";
    revisions = {
      r0 = {
        nix = import ../hackage/circuit-notation-0.2.0.0-r0-a9385711f2fc8eb602b556b4ef3b9b2f3f26ad42eacc6f854555b3c8d6188cc6.nix;
        revNum = 0;
        revTimestamp = "2026-04-23T14:15:55Z";
        sha256 = "a9385711f2fc8eb602b556b4ef3b9b2f3f26ad42eacc6f854555b3c8d6188cc6";
      };
      default = "r0";
    };
  };
}