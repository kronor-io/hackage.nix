{
  "0.1.0.0" = {
    sha256 = "8a1a6e090aa3d527e41a8f78b743da85a64a6bcb2acdcb67d37c85d2f99ec845";
    revisions = {
      r0 = {
        nix = import ../hackage/gruvbox-colors-0.1.0.0-r0-b243b04da0bcc09f79bb36d8cd20a989a29256b21cf8b055cac64615f443010a.nix;
        revNum = 0;
        revTimestamp = "2025-04-01T13:14:34Z";
        sha256 = "b243b04da0bcc09f79bb36d8cd20a989a29256b21cf8b055cac64615f443010a";
      };
      default = "r0";
    };
  };
  "0.1.0.1" = {
    sha256 = "c147c1867ffaa73ec970241479f015658be6242b4fbe669bc08ebd348391e3d7";
    revisions = {
      r0 = {
        nix = import ../hackage/gruvbox-colors-0.1.0.1-r0-fd1c3273d16b5646569afbde6b055c495a076e4bbab1211d42f094256d7afe37.nix;
        revNum = 0;
        revTimestamp = "2026-06-03T10:53:49Z";
        sha256 = "fd1c3273d16b5646569afbde6b055c495a076e4bbab1211d42f094256d7afe37";
      };
      default = "r0";
    };
  };
}