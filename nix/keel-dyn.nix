{
  "0.1.0.0" = {
    sha256 = "5e0fa8ada9c66b91450a6ad033fee62a5f4b7ac1f48c663ae8d9dc51d27fa461";
    revisions = {
      r0 = {
        nix = import ../hackage/keel-dyn-0.1.0.0-r0-bf4e8f52f2d736338341f96e69de876e6a3b4e4d8410bbd77fa68451ea0cc431.nix;
        revNum = 0;
        revTimestamp = "2026-08-19T23:30:26Z";
        sha256 = "bf4e8f52f2d736338341f96e69de876e6a3b4e4d8410bbd77fa68451ea0cc431";
      };
      default = "r0";
    };
  };
  "0.1.1.0" = {
    sha256 = "843ad90222a68b53096e764f5aeae42dd0fe24a5e734af20f9b1809840f2c43b";
    revisions = {
      r0 = {
        nix = import ../hackage/keel-dyn-0.1.1.0-r0-7b4deddda0cdaa238d71975a671afb81cd3c7d99cc76ba658406aab8b13f2b4b.nix;
        revNum = 0;
        revTimestamp = "2026-09-03T12:55:25Z";
        sha256 = "7b4deddda0cdaa238d71975a671afb81cd3c7d99cc76ba658406aab8b13f2b4b";
      };
      default = "r0";
    };
  };
}