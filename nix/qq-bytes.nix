{
  "1.0.0.0" = {
    sha256 = "bf5cca4c344f212a84ddee18d03329c37399d98f6f0c62fd04d65dd1b59fae2c";
    revisions = {
      r0 = {
        nix = import ../hackage/qq-bytes-1.0.0.0-r0-f191fac6066b8e8006ff92eb4d7fd18df24a471fb536d33b2f8828bf7f6c4b6f.nix;
        revNum = 0;
        revTimestamp = "2024-09-26T12:59:23Z";
        sha256 = "f191fac6066b8e8006ff92eb4d7fd18df24a471fb536d33b2f8828bf7f6c4b6f";
        };
      default = "r0";
      };
    };
  "1.0.0.1" = {
    sha256 = "fe4fe4ce6d90c3478059542a3ab26310795a9e0ab7e2358bc6730972d73e6c04";
    revisions = {
      r0 = {
        nix = import ../hackage/qq-bytes-1.0.0.1-r0-683dbede8fdb784dd337c752adfb0bea392ecc26a368b1146999dd98a9d61af6.nix;
        revNum = 0;
        revTimestamp = "2025-02-23T22:52:03Z";
        sha256 = "683dbede8fdb784dd337c752adfb0bea392ecc26a368b1146999dd98a9d61af6";
        };
      default = "r0";
      };
    };
  }