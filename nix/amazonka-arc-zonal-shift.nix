{
  "2.0" = {
    sha256 = "edbdb946fb73ce49c08a7d506e93622d8b406e0dd01c4d26696c26ab42ed7f71";
    revisions = {
      r0 = {
        nix = import ../hackage/amazonka-arc-zonal-shift-2.0-r0-d8b627959d203202306b9813d1cec0e1f5ca066f859eb7976be7339b63aca671.nix;
        revNum = 0;
        revTimestamp = "2023-07-27T21:05:27Z";
        sha256 = "d8b627959d203202306b9813d1cec0e1f5ca066f859eb7976be7339b63aca671";
        };
      r1 = {
        nix = import ../hackage/amazonka-arc-zonal-shift-2.0-r1-9ced34476736213665ba36b2f8a884d9092d54b8d8313f65b42f5eacf6961ba4.nix;
        revNum = 1;
        revTimestamp = "2024-05-13T07:34:59Z";
        sha256 = "9ced34476736213665ba36b2f8a884d9092d54b8d8313f65b42f5eacf6961ba4";
        };
      default = "r1";
      };
    };
  }