{
  "0.1.0.0" = {
    sha256 = "1b5025489402bac86eff7b9f504346e65f6ded428cf62043ba04dbb59e161fa4";
    revisions = {
      r0 = {
        nix = import ../hackage/sha-validation-0.1.0.0-r0-412c651174f56a41eed68b9a6d97647b6798b9ac654bfa230db66042b553a41c.nix;
        revNum = 0;
        revTimestamp = "2022-08-20T16:10:17Z";
        sha256 = "412c651174f56a41eed68b9a6d97647b6798b9ac654bfa230db66042b553a41c";
        };
      default = "r0";
      };
    };
  "0.1.0.1" = {
    sha256 = "6f0a4c3254b44670abd66ee40fceb7409db5a94ff29dc53eba528ac532fa4ea7";
    revisions = {
      r0 = {
        nix = import ../hackage/sha-validation-0.1.0.1-r0-a6bcbc5b565c220bcb18837d5b71c1e382583aa6c2b14ee8494facf2daec47d1.nix;
        revNum = 0;
        revTimestamp = "2024-07-06T13:08:39Z";
        sha256 = "a6bcbc5b565c220bcb18837d5b71c1e382583aa6c2b14ee8494facf2daec47d1";
        };
      default = "r0";
      };
    };
  }