{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "2.4";
      identifier = { name = "recover-rtti"; version = "0.5.0"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "9.4") (hsPkgs."data-array-byte" or (errorHandler.buildDepError "data-array-byte"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).ge "9.10") (hsPkgs."ghc-internal" or (errorHandler.buildDepError "ghc-internal"));
        buildable = true;
        };
      tests = {
        "test-recover-rtti" = {
