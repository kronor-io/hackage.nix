{ system
  ...
  , ... }:
  {
    flags = { ghcdebug = false; };
    package = {
      specVersion = "1.12";
      identifier = { name = "hledger-lib"; version = "1.40"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (!system.isWindows) (hsPkgs."pager" or (errorHandler.buildDepError "pager"))) ++ (pkgs.lib).optional (flags.ghcdebug) (hsPkgs."ghc-debug-stub" or (errorHandler.buildDepError "ghc-debug-stub"));
        buildable = true;
        };
      tests = {
        "doctest" = {
