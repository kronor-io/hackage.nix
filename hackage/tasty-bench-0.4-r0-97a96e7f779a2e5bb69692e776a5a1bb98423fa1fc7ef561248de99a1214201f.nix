{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "1.18";
      identifier = { name = "tasty-bench"; version = "0.4"; };
      license = "MIT";
    ...
    components = {
      "library" = {
        depends = (([
          ...
          ]) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "7.8") (hsPkgs."tagged" or (errorHandler.buildDepError "tagged"))) ++ (pkgs.lib).optional (compiler.isGhc && (compiler.version).lt "8.4") (hsPkgs."time" or (errorHandler.buildDepError "time"));
        buildable = true;
        };
      benchmarks = {
        "bench-fibo" = {
