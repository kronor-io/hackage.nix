{ system
  ...
  , ... }:
  {
    flags = { ghcdebug = false; terminfo = true; threaded = true; };
    package = {
      specVersion = "1.12";
      identifier = { name = "hledger"; version = "1.40"; };
    ...
    components = {
      "library" = {
        depends = ([
          ...
          ] ++ (pkgs.lib).optional (!system.isWindows && flags.terminfo) (hsPkgs."terminfo" or (errorHandler.buildDepError "terminfo"))) ++ (pkgs.lib).optional (flags.ghcdebug) (hsPkgs."ghc-debug-stub" or (errorHandler.buildDepError "ghc-debug-stub"));
        buildable = true;
        };
      exes = {
        "hledger" = {
